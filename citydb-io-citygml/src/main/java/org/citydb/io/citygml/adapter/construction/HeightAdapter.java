...
public class HeightAdapter implements ModelBuilder<Height, Attribute>, ModelSerializer<Attribute, Height> {

    @Override
    public void build(Height source, Attribute target, ModelBuilderHelper helper) throws ModelBuildException {
        helper.addAttribute(Name.of("value", Namespaces.CONSTRUCTION), source.getValue(), target, LengthAdapter.class);

        if (source.getStatus() != null) {
            target.addProperty(Attribute.of(Name.of("status", Namespaces.CONSTRUCTION), DataType.STRING)
                    .setStringValue(source.getStatus().toValue()));
        }

        helper.addAttribute(Name.of("lowReference", Namespaces.CONSTRUCTION), source.getLowReference(), target,
                CodeAdapter.class);
        helper.addAttribute(Name.of("highReference", Namespaces.CONSTRUCTION), source.getHighReference(), target,
                CodeAdapter.class);
    ...
    @Override
    public Height createObject(Attribute source) throws ModelSerializeException {
        return new Height();
    }

    @Override
    public void serialize(Attribute source, Height target, ModelSerializerHelper helper) throws ModelSerializeException {
