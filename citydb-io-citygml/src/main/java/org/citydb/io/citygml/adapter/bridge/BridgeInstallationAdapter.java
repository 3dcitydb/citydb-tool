...
@DatabaseType(name = "BridgeInstallation", namespace = Namespaces.BRIDGE)
public class BridgeInstallationAdapter extends AbstractInstallationAdapter<BridgeInstallation> {

    @Override
    public Feature createModel(BridgeInstallation source) throws ModelBuildException {
        return Feature.of(FeatureType.BRIDGE_INSTALLATION);
    }

    @Override
    public void build(BridgeInstallation source, Feature target, ModelBuilderHelper helper) throws ModelBuildException {
        super.build(source, target, helper);
        helper.addStandardObjectClassifiers(source, target, Namespaces.BRIDGE);

        if (source.hasDeprecatedProperties()) {
            DeprecatedPropertiesOfBridgeInstallation properties = source.getDeprecatedProperties();
    ...
    @Override
    public BridgeInstallation createObject(Feature source) throws ModelSerializeException {
        return new BridgeInstallation();
    }

    @Override
    public void serialize(Feature source, BridgeInstallation target, ModelSerializerHelper helper) throws ModelSerializeException {
        super.serialize(source, target, helper);
        helper.addStandardObjectClassifiers(source, target, Namespaces.BRIDGE);
    }

    @Override
    public void postSerialize(Feature source, BridgeInstallation target, ModelSerializerHelper helper) throws ModelSerializeException {
    ...
    @Override
    protected void configureSerializer(SpaceGeometrySupport<BridgeInstallation> geometrySupport) {
        BiFunction<CityGMLVersion, BridgeInstallation, Boolean> predicate =
                (version, target) -> version == CityGMLVersion.v2_0
                        && target.getRelationToConstruction() != RelationToConstruction.INSIDE;
