...
public class GeometryReferenceResolver {
    ...
    void resolveGeometryReferences(AbstractFeature feature, int featureId) {
        if (!references.isEmpty()) {
            ...
            try (CopySession session = geometryCopier.createSession()) {
                new ResolverProcessor(featureId, geometryCopier, session).resolve(feature);
            }
        }
    }
    ...
    private class ResolverProcessor extends ObjectWalker {
        ...
        private final GeometryCopier geometryCopier;
        private final CopySession session;

        ResolverProcessor(int featureId, GeometryCopier geometryCopier, CopySession session) {
            this.featureId = featureId;
            this.geometryCopier = geometryCopier;
