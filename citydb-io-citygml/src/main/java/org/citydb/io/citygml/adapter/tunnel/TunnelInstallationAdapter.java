...
@DatabaseType(name = "TunnelInstallation", namespace = Namespaces.TUNNEL)
public class TunnelInstallationAdapter extends AbstractInstallationAdapter<TunnelInstallation> {

    @Override
    public Feature createModel(TunnelInstallation source) throws ModelBuildException {
        return Feature.of(FeatureType.TUNNEL_INSTALLATION);
    }

    @Override
    public void build(TunnelInstallation source, Feature target, ModelBuilderHelper helper) throws ModelBuildException {
        super.build(source, target, helper);
        helper.addStandardObjectClassifiers(source, target, Namespaces.TUNNEL);

        if (source.hasDeprecatedProperties()) {
            DeprecatedPropertiesOfTunnelInstallation properties = source.getDeprecatedProperties();
    ...
    @Override
    public TunnelInstallation createObject(Feature source) throws ModelSerializeException {
        return new TunnelInstallation();
    }

    @Override
    public void serialize(Feature source, TunnelInstallation target, ModelSerializerHelper helper) throws ModelSerializeException {
        super.serialize(source, target, helper);
        helper.addStandardObjectClassifiers(source, target, Namespaces.TUNNEL);
    }

    @Override
    public void postSerialize(Feature source, TunnelInstallation target, ModelSerializerHelper helper) throws ModelSerializeException {
    ...
    @Override
    protected void configureSerializer(SpaceGeometrySupport<TunnelInstallation> geometrySupport) {
        BiFunction<CityGMLVersion, TunnelInstallation, Boolean> predicate =
